...
| JST-PH 3-pin (J6) | NeoPixel LED strip connector | Real footprint (`JST_PH_B3B-PH-K_1x03_P2.00mm_Vertical`), user's choice for easy connect/disconnect. **J7 (was second/inner-ring connector) removed from scope 2026-08-06** -- user is sourcing a single LED strip only, no second ring. Schematic/PCB still show J7 as of this writing; removing it is the next concrete PCB task (see Work Packages WP3.1) -- deliberately deferred rather than hand-edited blind at the tail end of the 2026-08-05/06 GUI session, see Session Log for why. Also worth doing in the GUI schematic editor when picked back up, not another headless S-expression edit -- deleting a component and updating a net is a simple, low-risk GUI operation that keeps schematic/PCB properly in sync via KiCad's normal update flow. |
