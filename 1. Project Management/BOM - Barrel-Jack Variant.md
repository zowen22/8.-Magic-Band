...
| J2 | RFID reader socket | **2.54MM 1X8P RED**, [JLCPCB C9900138233](https://jlcpcb.com/partdetail/JLCPCBAssembly-2_54MM_1X8PRED/C9900138233) — fab-placed | **Socket itself resolved 2026-09-21** — moved from hand-solder back to fab-placed (see Assembly Service section). The MFRC522 module that plugs into it is a separate purchase, any standard breakout (widely sold on Amazon — HiLetgo, SunFounder, etc.). **Confirmed 2026-08-21: standard module size is 40x60mm** — the overhang risk flagged in `PCB Design Plan.md` SS1.2 is real, not hypothetical, for a 40x40mm board. Not a functional problem (it stacks as a shield via the 1x08 socket, doesn't need to fit *within* the board outline) but the module will physically overhang by ~20mm in one dimension — factor that into shell/enclosure clearance |
...
**Revised again 2026-09-21**: user reversed the J2 half of the above — J2 (MFRC522 socket) moved back to fab-placed. The mechanical-reliability reasoning from 2026-08-25 was a preference, not a technical blocker (mixed THT assembly always supported it fine), and a plain 1x8 female 2.54mm socket is genuinely generic. Un-flagged `dnp` on J2 in both the schematic and PCB (matching the exact reverse of the 2026-08-25 change), verified ERC/DRC unchanged (129/7, 0 unconnected — same baseline, pure assembly-intent flag). Real catalog part found: **2.54MM 1X8P RED, JLCPCB C9900138233**, Extended tier, THT, 1x8/2.54mm — confirmed matches `PinSocket_1x08_P2.54mm_Vertical` exactly (color is cosmetic, hidden once the MFRC522 module stacks on top). **J3 stays hand-solder** — not revisited, still the user's call from 2026-08-25.

**Fab-placeable (17 refs)**: R1, R2, C1-C8 (all passives), Q1, U2 (SMD ICs), Y1 (crystal, THT), U1 (MCU, THT), J2 (socket, THT — C9900138233), J6/J9 (connectors, THT — keyed/pre-built mating cables). JLCPCB and similar services support mixed SMT+THT assembly in one order (SMD via reflow, THT via wave/selective soldering) — confirmed via their own docs, not assumed.
...
- **J3 (RF TX header)** — marked `DNP` in the schematic and PCB (2026-08-25). *Could* be fab-placed (mixed THT assembly supports it fine), but user prioritized solder-joint stability on a connector that takes repeated mechanical stress (wire tension from the RF TX module's leads) over the convenience of one-shot assembly. J2 was in this same category until reversed 2026-09-21 (see above).
