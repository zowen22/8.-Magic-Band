...
Organized as a proper multi-level assembly BOM: **Level 1** = top-level items that go into the finished ornament (bold, with a subtotal) — the PCB Assembly is one of these, siblings to the off-board accessories. **Level 2** (↳) = what's directly attached to/plugged into the PCB Assembly — the bare fabricated board itself, plus modules and connectors soldered onto it (matches the schematic's `J`-numbered reference designators: sockets, headers, jacks). **Level 3** (↳↳) = the individual components populated directly on the bare board (matches `U`/`Y`/`C`/`R`/`Q`-numbered designators).

| Level | Component | Part | Price | Notes |
|---|---|---|---|---|
| **1** | **PCB Assembly** | — | **~$7.75–17.40** *(excl. fab)* | Everything soldered onto/into the custom board |
| 2 | ↳ Bare PCB | 40mm × 40mm, 2-layer, fabricated | **Not yet priced** | Board outline hit its 40×40mm target with zero courtyard overlaps. Get a real JLCPCB quote once Gerbers are finalized (blocked on the RF TX footprint below) |
| 3 | ↳↳ Microcontroller | ATmega328P-PU, DIP-28, bare chip | ~$2.50–4 | **U1.** Replaces Pro Mini/Nano from earlier variants — no onboard USB/regulator, hand-solderable/socketable DIP package chosen deliberately (no reflow needed) |
| 3 | ↳↳ Crystal | 16MHz, HC-49 or similar | ~$0.50–1 | **Y1.** Load-cap value (C1/C2, currently 22pF placeholder) needs confirming against the actual part's datasheet once sourced |
| 3 | ↳↳ Load caps | 22pF ×2 (C1, C2) | ~$0.05–0.10 ea (bulk) | Crystal load caps — see above, value is generic pending real crystal spec |
| 3 | ↳↳ Decoupling caps | 100nF ×3 (C3, C4, C_AREF), 1µF (C5), 10µF (C6), 1.0µF (C7) | ~$0.05–0.15 ea (bulk) | Standard MCU/regulator decoupling. C5 was flagged and fixed to meet MCP1700's 1.0µF datasheet minimum during PDR review |
| 3 | ↳↳ Reverse-polarity protection | Q1 — AO3401A P-MOSFET (SOT-23) + R1, R2 (10k) | ~$0.10–0.30 (Q1) + negligible (resistors) | Ideal-diode protection ahead of the regulator/MCU — barrel jack is bare flying leads with no mechanical keying, so reversed/wrong-voltage adapters are a real risk. Chosen over a series diode to avoid eating into the 16MHz ATmega's voltage headroom |
| 3 | ↳↳ 3.3V Regulator | MCP1700-3302 LDO (SOT-23) | ~$0.50–1/unit (10–20 packs) | **U2.** Steps 5V down to 3.3V for MFRC522 + RF TX module. Same part/rationale as the wireless-variant BOM |
...
| **1** | **RF Outlet** | Etekcity or similar 433MHz outlet kit | **~$25–30** | Off-board accessory, pre-paired before shipping — not part of the PCB Assembly |
| **1** | **Power Supply** | 5V 3A wall adapter, barrel jack output | **~$8–12** | Buyer-supplied or included; standard commodity, well above this board's actual draw. Mates with the PCB Assembly's power input (J8) but isn't part of it |
| **1** | **Programmer** *(one-time tooling)* | USBasp or spare Nano-as-ISP | **~$3–5** | Not per-unit — bench tooling only, used to flash via the ISP header (J9) |

**Per-unit total (PCB Assembly + RF Outlet + Power Supply, excl. bare-PCB fab cost + one-time programmer): ~$41–59**
**PCB Assembly alone: ~$7.75–17.40 of that**

### Optional add-on: Audio (Level 1, not on this PCB — wired off-board, same as Nano build)
