...
| J8 | Barrel jack | 5.5mm/2.1mm, **center-positive**, threaded panel-mount, pre-wired — **ordered 2026-09-21** ([DaierTek pre-wired panel-mount jack](https://www.amazon.com/5-5x2-1MM-Pre-Wired-Connector-Waterproof-Appliances/dp/B0BD46CP5Y)) | **Not board-mounted** — bare flying leads hand-soldered to J8's 2 THT pads (see `PCB Design Plan.md` SS1.5). **Center pin = +5V, sleeve = GND — verify this on the physical jack before soldering, don't cross the wires.** This board's own history includes catching a real reverse-polarity risk from a mismatched symbol, so this isn't a throwaway reminder |
...
| J6 | NeoPixel LED strip | WS2812B, 5V — **@user has LEDs in hand as of 2026-09-21**, exact product/length not recorded | **Length/pixel-count decision still open** (flagged since 2026-08-06) — deliberately deferred by @user, revisit before final assembly/firmware pixel-count config. **Pigtail resolved 2026-09-21**: @user bought a [JST-PH 2.0mm 3-pin pigtail set](https://www.amazon.com/dp/B0DG8VLJF9) (male+female, cable attached, sold for LED strip use) — confirmed correct spec, use the female half to crimp/solder onto the strip's cut end so it plugs into J6 |
...
- ~~Y coordinate negated from kicad-cli's raw export~~ — **wrong, corrected 2026-09-21, see below.**
...
`MagicBand_BarrelJack_CPL.csv` now has the corrected format directly (kicad-cli's raw output was post-processed via a script, not committed separately).

**2026-09-21 correction — the Y-negation above was wrong, and stayed wrong for a month undetected.** The original "confirmed correct" check (values land inside the board's 0-40mm bounds, matching a generic sample template's all-positive values) was too weak to catch a sign error — the sample template belonged to a *different* board and proved nothing about this one's actual native frame. Real root cause found via @user's live cursor readout on JLCPCB's own placement-preview screen: J8 (bare pads, no CPL entry, so its on-screen position is read straight off the board/Gerber render with zero involvement from our CPL) showed **Y = -36**, an exact match to kicad-cli's raw, *un-negated* PosY for J8. Meanwhile J6 and J9 (both CPL-placed) showed *positive* Y on screen, matching our then-negated CPL — i.e., every CPL-placed part was landing at the mirror-opposite Y from the board's own true native frame the whole time. **Fix: use kicad-cli's raw PosY directly, no negation at all** — `MagicBand_BarrelJack_CPL.csv` regenerated accordingly and diffed to confirm it's a clean sign-flip across every row from the old (wrong) version. No board was ever fabricated with the old file, but the resulting assembly would have had every through-hole and SMD component mirrored top-to-bottom relative to its actual footprint had it gone through. **Lesson for any future coordinate-format verification on this project: check against a live readout of the actual board's own real geometry, not a generic sample/template's value range** — "lands in a plausible-looking range" is not the same as "matches this board's true frame."

**Rotation corrections, 2026-09-21 (after the position fix above)**: `Rotation normalized to 0-360` alone wasn't enough — 5 of the 19 parts needed real per-part rotation corrections beyond that, found by @user comparing the live JLCPCB render against reality one part at a time (this matches a well-documented KiCad→JLCPCB pain point: rotation reference is tied to the specific matched catalog part's own 3D model, not a property of the KiCad footprint name, so there's no single global formula — see the Fabrication Toolkit/kicad-jlcpcb-tools community tooling, which maintains large per-part correction-table databases for exactly this reason). Confirmed **clockwise = decreasing the rotation value** on this board's rendering (established via J9: a first guess of +90 was confirmed backwards, corrected to the real answer of 270, i.e. -90). Final corrections: U2 270→90, Q1 0→180, J9 0→270, U1 180→90, J6 180→0. **Y1 (crystal) needs no rotation fix at all** — confirmed genuinely orientation-independent (plain 2-lead symmetric passive, no case-ground tab, both load caps the same 27pF value just different packages) rather than checked and found already-correct. C1-C8/R1/R2 (simple passives) and J2/J3/J8 (DNP) were never flagged.
...
- **J6, J9 resolved 2026-09-05** — real catalog matches found for both, closing out the last open item before checkout:
  ...
  - **Mating cable plan (for repeated multi-board ISP flashing, not just this one board)**: user is flashing multiple boards via a spare Arduino Nano running the ArduinoISP sketch, not a dedicated USBasp. Buy one generic 6-pin 2x3 IDC ribbon cable with female sockets on both ends — cut the connector off one end, strip and solder those 6 wires directly to the Nano's D10/D11/D12/D13/5V/GND pins once; the other end's factory female IDC socket plugs onto every board's J9 going forward, keyed so it can't go in misaligned. See `4. Technical Reference.md`'s new ISP Programming section for the full pin mapping and wiring gotchas. **Resolved 2026-09-21**: @user ordered a [MECCANIXITY 2-pack, 6-pin/2.54mm/FC-FC/30cm](https://www.amazon.com/dp/B0GJ6GPR9K) — confirmed correct spec, includes a spare cable.
